...
def load_data(file_path):
    ...
    data['time_decay'] = np.exp(-data['time_decay'] / 14)

    data['price'] = (data['price'] - data['price'].min()) / (data['price'].max() - data['price'].min())
    data['rating'] *= data['time_decay']
    data['rating'] += data.groupby(['userId', 'productId'])['action'].transform('count') * 0.5
    data['rating'] = data['rating'].clip(3, 5)
    return data
...
def get_recommendations():
    ...
    predictions.sort(key=lambda x: x[1], reverse=True)
    collaborative_recommendations = [pid for pid, _ in predictions[:20]]
    # Generăm recomandări pe bază de conținut
    content_recommendations = content_based_recommendations(data, collaborative_recommendations[0], top_n=20)
    
    # Combinăm recomandările
    hybrid_recommendations = list(set(collaborative_recommendations + content_recommendations))[:20]
    return jsonify({
        "user_id": user_id,
